...
class PoolFormerBlock(nn.Module):
    ...
    def __init__(self, dim, pool_size=3, mlp_ratio=4.,
        ...
        # self.token_mixer = Pooling(pool_size=pool_size)  # 原始池化
        self.token_mixer = nn.Conv3d(in_channels=dim, out_channels=dim, kernel_size=1, stride=1, padding=0)
...
class PoolFormer(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None,
        ...
        if self.fork_feat:
            ...
            self.head = nn.Linear(
                embed_dims[-1], out_chans) if out_chans > 0 \
                else nn.Identity()
        ...
        # @jy
        self.out_conv = nn.Conv3d(in_channels=embed_dims[-1], out_channels=in_chans, kernel_size=1, stride=1)
