...
import sys
# from net.asmlp.shift_cuda import Shift
...
class AxialShift(nn.Module):
    ...
    def forward(self, x):
        ...
        x_shift_lr = torch_shift(x, self.shift_size, 3)
        x_shift_td = torch_shift(x, self.shift_size, 2)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)  # 非重叠patch
...
class ASMLP(nn.Module):
    def __init__(self, img_size=16, patch_size=4, in_chans=3,
                 embed_dim=768, depths=[2, 2, 6, 2],
...
class AsmlpBlock(nn.Module):
    def __init__(self, img_size=16, patch_size=1, in_chans=512, out_chans=512, embed_dim=768, depth=18, shift_size=5):
        super(AsmlpBlock, self).__init__()
        self.asmlp = ASMLP(img_size=img_size, patch_size=patch_size, in_chans=in_chans* img_size, embed_dim=embed_dim,
    ...
    def forward(self, x):
        ...
        # df_wd = self.asmlp(df_wd).reshape(B, H, C, W, D).permute(0, 2, 1, 3, 4)  # ->(B, C, H, W, D)
        # # H-D维度
        # df_hd = x.permute(0, 3, 1, 2, 4).reshape(B * W, C, H, D)  # -> (B*W, C, H, D)
        # df_hd = self.asmlp(df_hd).reshape(B, W, C, H, D).permute(0, 2, 1, 3, 4)  # ->(B, C, H, W, D)
        # # H-W维度
        # df_hw = x.permute(0, 4, 1, 2, 3).reshape(B * D, C, H, W)
        # df_hw = self.asmlp(df_hw).reshape(B, D, C, H, W).permute(0, 2, 3, 4, 1)  # ->(B, C, H, W, D)
...
class AsmlpBlockMerge2Batch(nn.Module):
    def __init__(self, img_size=16, patch_size=1, in_chans=512, out_chans=512, embed_dim=768, depth=18, shift_size=5):
        super(AsmlpBlockMerge2Batch, self).__init__()
        self.asmlp = ASMLP(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
                          depths=[depth], shift_size=shift_size)
...
if __name__ == '__main__':
    ...
    model = ASMLP(img_size=30, patch_size=1, in_chans=512, embed_dim=512,
                  depths=[12], shift_size=3)

    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f'There are {n_params} trainable parameters.')

    x = torch.randn([1, 512, 30, 30]).cuda()
    model.cuda()
    y = model(x)
